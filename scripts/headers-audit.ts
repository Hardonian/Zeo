...
for (const path of requiredPaths) {
  ...
  if (!csp && !cspReportOnly) {
    failures.push(`${path}: missing CSP header`);
  }
  if (!response.headers.get('x-content-type-options')?.includes('nosniff')) {
    failures.push(`${path}: missing X-Content-Type-Options nosniff`);
  }
  if (!response.headers.get('referrer-policy')) {
    failures.push(`${path}: missing Referrer-Policy`);
  }
  if (!response.headers.get('permissions-policy')) {
    failures.push(`${path}: missing Permissions-Policy`);
  }
  ...
  if (!frameAncestorsProtected) {
    failures.push(`${path}: missing clickjacking protection (X-Frame-Options/CSP frame-ancestors)`);
  }

  const expectEnforce = process.env.CSP_MODE === 'enforce';
  if (expectEnforce && !csp) {
    failures.push(`${path}: CSP_MODE=enforce requires Content-Security-Policy header`);
  }
  if (!expectEnforce && !cspReportOnly) {
    failures.push(`${path}: CSP should remain report-only unless CSP_MODE=enforce`);
  }
}
