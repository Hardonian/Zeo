 ...
 */
export function applyCalibrationWiden(prediction, domain, config) {
    if (!config.enabled) {
        return prediction;
    }
 ...
 */
export function configFromRecommendation(recommendation, sourceDatasetId, sourceReportHash) {
    return {
        enabled: true,
 ...
 */
export function applyCalibrationToPredictions(predictions, domainResolver, config) {
    if (!config.enabled) {
        return predictions;
    }
 ...
 */
export function widenBand(originalBand, widenFactor) {
    // v0.3.1: Only widen, never narrow
    if (widenFactor <= 1.0) {
        return originalBand;
    }
 ...
 */
export function wouldCalibrationChange(prediction, domain, config) {
    if (!config.enabled) {
        return false;
    }
    const widenFactor = config.widenFactorByDomain[domain] ?? config.widenFactorDefault;
...
//# sourceMappingURL=calibration-feedback.js.map