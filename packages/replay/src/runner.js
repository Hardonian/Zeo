 ...
 */
export async function replayCase(replayCase, options) {
    ...
    const sortedBatches = [...replayCase.observationBatches].sort((a, b) => a.timestamp.localeCompare(b.timestamp));
    ...
    const trackedMetrics = replayCase.outcome.metrics.map((m) => ({
        metricId: m.metricId,
        targetKind: m.mapping.linksTo,
        targetId: m.mapping.targetId,
    }));
    // Checkpoint at asOf time (before any observations)
    const asOfCheckpoint = await createCheckpoint(replayCase.horizons.asOf, replayCase.decisionSpec, [], decisionHash, observationsHash, seed, ENGINE_VERSION, trackedMetrics, options);
    ...
    for (const batch of sortedBatches) {
        ...
        const checkpoint = await createCheckpoint(batch.timestamp, replayCase.decisionSpec, batchesUpTo, decisionHash, batchHash, seed, ENGINE_VERSION, trackedMetrics, options);
...
async function createCheckpoint(at, decisionSpec, batchesUpTo, decisionHash, observationsHash, seed, engineVersion, trackedMetrics) {
    ...
    for (const batch of batchesUpTo) {
        const batchObservations = batch.observations;
    ...
    const specWithAssumptions = decisionSpec;
    ...
    const predictions = buildPredictionBundle(at, { variables: mockVariables }, decisionHash, observationsHash, seed, engineVersion, trackedMetrics);
    return {
        at,
        posteriorSummary,
        predictions,
 ...
 */
function computeCalibrationScore(checkpoints, outcome) {
    ...
    for (const metric of outcome.metrics) {
        const prediction = predictions.find((p) => p.target.id === metric.metricId || p.target.id === metric.mapping.targetId);
        if (prediction) {
    ...
    const overall = coverageValues.reduce((sum, c) => sum + c, 0) / Math.max(coverageValues.length, 1);
    // Compute recommended adjustments
    const widenFactorOverall = computeWidenFactor(overall);
    const widenFactorByDomain = {};
 ...
 */
function computeMetricCoverage(metric, prediction) {
    switch (metric.kind) {
        case "binary": {
            const binaryValue = metric.value;
 ...
 */
function computeWidenFactor(coverage) {
    // Target 90% coverage
 ...
 */
function createEmptyCalibrationScore() {
    return {
        coverage: {
            byMetricId: {},
            byDomain: {},
