...
export class CounterfactualEngine {
     ...
     */
    generateCounterfactuals(decision, alternativeActions) {
        const scenarios = [];
        for (const action of alternativeActions) {
            // Skip the action that was actually taken
            if (action.id === decision.branchRecord.selectedActionId)
                continue;
     ...
     */
    buildCounterfactual(decision, action) {
     ...
     */
    inferPlausibleOutcomes(decision, action) {
     ...
     */
    calculateCounterfactualConfidence(decision, action) {
     ...
     */
    identifyCriticalUncertainties(decision, action) {
        return [
     ...
     */
    generateCounterfactualRationale(decision, action, confidence) {
     ...
     */
    analyzeRegret(decision, counterfactuals) {
        ...
        // Generate insight
        const insight = this.generateRegretInsight(decision, actualOutcome, outcomeQuality, decisionQuality, regrets);
        return {
            decisionId: decision.id,
     ...
     * Calculate regret metrics.
     */
    calculateRegretMetrics(decision, counterfactuals) {
        ...
        const counterfactualValues = counterfactuals.map(cf => {
            const avgProbability = cf.plausibleOutcomes.reduce((sum, o) => sum + (o.probability.low + o.probability.high) / 2, 0) / cf.plausibleOutcomes.length;
     ...
     */
    assessDecisionQuality(decision, counterfactuals) {
     ...
     */
    generateRegretInsight(decision, outcome, outcomeQuality, decisionQuality, regrets) {
        let insight = "";
        // Distinguish bad outcome from bad decision
     ...
     */
    identifyCriticalInformation(decision, counterfactuals) {
        return [
     ...
     */
    replayDecision(decision, currentKnowledge) {
        return {
            atTime: {
