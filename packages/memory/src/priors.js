...
export class PriorUpdateEngine {
     ...
     */
    updateFromOutcome(decision, outcome, assumptionType) {
        const newUpdates = [];
        ...
        const outcomeType = wasCorrect ? "confirmed" :
     ...
     */
    applyPriors(baseInterval, options) {
        let wideningFactor = 1.0;
        const sources = [];
        const applicablePriors = [];
        ...
        if (options.assumptionType) {
            filteredPriors = applicablePriors.filter(p => p.name.includes(options.assumptionType) ||
     ...
     */
    getOrCreatePrior(level, key, assumptionType) {
     ...
     */
    applyBayesianUpdate(prior, outcome) {
        const now = new Date().toISOString();
     ...
     */
    createUpdateRecord(prior, decision, assumptionType, outcome) {
     ...
     */
    generateUpdateRationale(prior, outcome, decision) {
...
//# sourceMappingURL=priors.js.map