...
export class CausalEngine {
    ...
    buildDAG(name, nodes, edges) {
        ...
        const dagNodes = nodes.map((n, index) => {
     ...
     */
    identifyBackdoorPaths(nodes, edges, treatmentId, outcomeId) {
    ...
    findAllPaths(adjacency, current, target, visited, path) {
        if (current === target) {
            return [[...path, current]];
        }
    ...
    findBlockingSet(nodes, edges, path, treatmentId) {
     ...
     */
    createPredictiveClaim(dag, antecedentId, consequentId, association) {
     ...
     */
    createCausalClaim(dag, treatmentId, outcomeId, data) {
        // Check if backdoor criterion can be satisfied
        const paths = dag.backdoorPaths.filter((p) => p.path[0] === treatmentId && p.path[p.path.length - 1] === outcomeId);
    ...
    estimateEffect(treatmentId, outcomeId, data, dag) {
        ...
        const se = Math.sqrt((this.variance(treatedOutcomes) / treatedOutcomes.length) +
    ...
    analyze(dag, data) {
        ...
        if (treatment && outcome) {
            ...
            claims.push(this.createPredictiveClaim(dag, treatment.id, outcome.id, { low: 0, high: 0 }));
