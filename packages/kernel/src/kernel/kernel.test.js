...
import { computeDecision, computeDecisionIR, computePlan, computePlanIR, computeDiff, kernelHash, createKernelRng, createKernelIdGenerator, IR_VERSION, KERNEL_VERSION, KERNEL_SCHEMA_VERSION, validateIRVersion, isDecisionIR, isPlanIR, DecisionStateMachine, ReplayStateMachine, } from "./index.js";
// ─── Test Helpers ────────────────────────────────────────────────────────
...
function makeTestConfig(seed = "test-seed-deterministic") {
    return {
        seed,
        floatPrecision: 10,
        maxDepth: 2,
...
function makeTestInput(seed = "test-seed-deterministic") {
    return {
        spec: makeTestSpec(),
 ...
 */
function forAll(count, generator, property, seed = "property-test-seed") {
    ...
    for (let i = 0; i < count; i++) {
        ...
        try {
            ...
            throw new Error(`Property failed on iteration ${i}:\n` +
                `  Input: ${JSON.stringify(value, null, 2).slice(0, 500)}\n` +
                `  Error: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
}
function generateRandomSpec(rng, index) {
    const numActions = rng.nextInt(1, 4);
    const numAssumptions = rng.nextInt(1, 5);
    const kinds = ["communicate", "verify", "delay", "change_terms"];
    const statuses = ["fact", "belief", "assumption", "unknown"];
