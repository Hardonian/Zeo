 ...
 */
export function validateNormalizedInput(input: unknown): ValidationResult {
  const errors: DeterminismError[] = [];
  checkCanonical(input, "", errors);
  return { valid: errors.length === 0, errors };
}

function checkCanonical(value: unknown, path: string, errors: DeterminismError[]): void {
  if (value === undefined) {
    errors.push({
 ...
 */
export function validateStableSerialization(obj: unknown): ValidationResult {
  const errors: DeterminismError[] = [];
 ...
 */
export function validateIROrdering(ir: IRNode): ValidationResult {
  const errors: DeterminismError[] = [];
 ...
 */
export function validateOutputHash(
  expected: string,
  actual: string,
): ValidationResult {
  const errors: DeterminismError[] = [];
 ...
 */
export function validateFloatBounds(
  value: number,
  min: number,
  max: number,
  path: string,
...
export function assertValid(result: ValidationResult, context: string): void {
  if (!result.valid) {
    ...
    throw new DeterminismValidationError(
      `Determinism violation at ${context}: ${summary}`,
      result.errors,
    );
  }
}
...
export class DeterminismValidationError extends Error {
  constructor(
    message: string,
    public readonly violations: DeterminismError[],
  ) {
    super(message);
