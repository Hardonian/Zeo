            ...
            it("does not import forbidden Node modules", () => {
                for (const forbidden of FORBIDDEN_IMPORTS) {
                    ...
                    if (found) {
                        ...
                        if (forbidden === "node:crypto" || forbidden === '"crypto"')
                            continue;
                ...
                for (let lineNum = 0; lineNum < lines.length; lineNum++) {
                    ...
                    for (const pattern of FORBIDDEN_PATTERNS) {
                        if (pattern.test(line)) {
                            throw new Error(`Forbidden pattern ${pattern} found in ${fileName}:${lineNum + 1}: ${line.trim()}`);
                        }
                    }
                }
            });
...
//# sourceMappingURL=forbidden-imports.test.js.map