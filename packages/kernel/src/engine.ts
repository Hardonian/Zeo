...
// Type shim for AssumptionTracker if not moving repro-pack yet
...
export type RunDecisionOpts = {
  ...
  quantEngine?: QuantEngineInterface; // DI Injection
...
export function runDecision(spec: DecisionSpec, opts?: RunDecisionOpts): DecisionResult {
  ...
  try {
    ...
    if (opts?.tracker) {
      ...
      // ... (other tracker calls omitted for brevity but should be kept if full fidelity needed)
    }
    ...
    graph = rawGraph;
    budgetManager.incrementSteps(rawGraph.nodes.length);
    ...
    if (opts?.useQuantEngine && opts?.quantEngine) {
      ...
      budgetManager.check(getPartialResult);
      const quantFlip = opts.quantEngine.generateFlipConditions(spec, evaluations);
    ...
    return {
      ...
      explanation: {
        why: [
          ...
          (opts?.useQuantEngine && opts?.quantEngine)
