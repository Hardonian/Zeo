...
class DeterministicContext {
    ...
    getRng() {
        if (!this._rng)
            throw new Error("Deterministic context not active");
        return this._rng;
    }
    getClock() {
        if (!this._clock)
...
export function getDeterministicContext() {
    return ctx;
}
export function isDeterministic() {
    return ctx.active;
}
export function activateDeterministicMode(config) {
    ctx.activate(config);
}
 ...
 */
export function getDeterministicIdCounter() {
    return ctx.getIdCounter();
}
/**
 * Set the deterministic ID counter value (for replay restoration)
 */
export function setDeterministicIdCounter(value) {
...
export function normalizeFloat(value) {
    if (ctx.active)
        return ctx.normalizeFloat(value);
    return value;
}
/**
 * Stable sort that guarantees deterministic ordering for equal elements
 */
export function stableSort(arr, compareFn) {
 ...
 */
export function withDeterministicMode(config, fn) {
