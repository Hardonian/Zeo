...
import { createKernelIdGenerator } from "./id.js";
...
function createNode(idGen, label, kind, notes, deps = []) {
    return { id: idGen.nextId(), label, kind, notes, dependencies: deps };
}
function createEdge(idGen, from, to, actionId, p, notes) {
    return { id: idGen.nextId(), from, to, actionId, probability: p, notes };
}
// ─── Branch Graph Generation ─────────────────────────────────────────────
function generateBranchGraph(idGen, spec, clockNow, maxDepth, maxBranchesPerAction) {
...
// ─── Flip Conditions ─────────────────────────────────────────────────────
function generateKernelFlipConditions(spec, evaluations) {
    ...
    for (const assumption of spec.assumptions) {
        ...
        if (!isFlagged && assumption.confidence === "high")
            continue;
...
export function computeDecision(input) {
    ...
    const graph = generateBranchGraph(idGen, spec, clockNow, config.maxDepth, config.maxBranchesPerAction);
    // Run evaluations
    const evaluations = [
...
// ─── computeDecisionIR ──────────────────────────────────────────────────
export function computeDecisionIR(input) {
    const output = computeDecision(input);
    const evidenceRequests = output.nextBestEvidence.map((e, i) => ({
        version: IR_VERSION,
        kind: "evidence_query",
        prompt: e.prompt,
...
// ─── computePlan ─────────────────────────────────────────────────────────
export function computePlan(input, budget) {
    const output = computeDecision(input);
    // Flip distances
...
// ─── computePlanIR ──────────────────────────────────────────────────────
export function computePlanIR(input, budget) {
...
function computeFlipDistances(spec, output) {
...
function computeVoiEstimates(spec, output, flipDistances) {
    const estimates = [];
...
export function computeDiff(a, b) {
    ...
    if (changedAssumptions.length > 0)
        summaryParts.push(`${changedAssumptions.length} assumption change(s)`);
    if (changedOutputs.length > 0)
        summaryParts.push(`${changedOutputs.length} output change(s)`);
