 ...
 */
export function bandUncertainty(band) {
 ...
 */
export function aggregateUncertainty(posterior, variableWeights = {}) {
 ...
 */
function simulateObservationOutcome(candidate, rng) {
    // Draw from reliability band
    const reliability = rng.uniform(candidate.reliabilityBand.low, candidate.reliabilityBand.high, 1)[0];
 ...
 */
function candidateToObservation(candidate, outcome) {
    return {
 ...
 */
function cognitiveLoadPenalty(load) {
    switch (load) {
        case "low": return 5;
 ...
 */
function computeTotalCost(candidate) {
 ...
 */
function estimateFlipRelevance(candidate, posterior) {
    // High relevance if:
 ...
 */
export function computeVoi(worldSpec, posterior, candidates, seed, options = {}) {
    ...
    const candidateResults = candidates.map(candidate => {
        ...
        for (let i = 0; i < numSimulations; i++) {
            ...
            const simPosterior = inferPosterior(modifiedSpec, [syntheticObservation], simSeed);
...
export function regimeAwareScoreMultiplier(regime) {
    if (!regime)
        return 1.0;
...
export function estimateRegimeDisambiguationPotential(candidate, posterior, regime) {
    if (!regime)
        return 0;
    let potential = 0;
...
export function wouldNarrowConfidenceBand(candidate, posterior, regime) {
    if (!regime)
        return false;
...
export function computeRegimeAwareVoi(worldSpec, posterior, candidates, seed, options = {}) {
    ...
    const regimeAwareCandidates = baseReport.candidates.map((candidate, index) => {
        const disambiguationPotential = estimateRegimeDisambiguationPotential(candidates[index], posterior, regime);
