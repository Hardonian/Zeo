 ...
 */
export function computeProvenanceQuality(provenance, sourceWeights = {}) {
    if (provenance.length === 0)
        return 0.1; // No provenance = low quality
    let totalQuality = 0;
    for (const pointer of provenance) {
        // Base quality by source
        const sourceQuality = sourceWeights[pointer.sourceId] ?? 0.5;
        // Check completeness
        const hasRequired = pointer.sourceId &&
            pointer.checksum &&
            pointer.capturedAt;
        if (!hasRequired) {
 ...
 */
function mergeObservationBands(bands) {
    if (bands.length === 0)
        return { low: 0, high: 1 };
    if (bands.length === 1)
        return { low: bands[0].low, high: bands[0].high };
    // Weight by quality
    ...
    const overlaps = bands.every(b1 => bands.some(b2 => b1 !== b2 &&
    ...
    const avgWidth = bands.reduce((sum, b) => sum + (b.high - b.low) * (b.quality / totalWeight), 0);
    return {
...
export function inferPosterior(worldSpec, observations, seed, sourceWeights) {
    ...
    const observationBandsByVariable = new Map();
    for (const observation of canonicalObs) {
        // Find matching observation model
        const model = worldSpec.observationModels.find(m => observation.signalId.match(m.provenancePattern.replace("*", ".*")));
        if (!model)
            continue;
        // Check quality threshold
        const quality = computeProvenanceQuality(observation.provenance, sourceWeights);
        if (quality < model.minQualityThreshold)
            continue;
        ...
        for (const varId of model.targetVariableIds) {
            ...
            if (!varState)
                continue;
            const newBand = applyObservationToBand(varState.posteriorBand, observation, model, quality, rng);
            ...
            varState.provenanceRefs.push(...observation.provenance.map(p => `${p.sourceId}:${p.checksum.slice(0, 8)}`));
        }
    }
 ...
 */
export function observationsToWorldEvidence(observations, evidenceEvents, mappingRules) {
    const result = [...observations];
    for (const event of evidenceEvents) {
        // Find matching mapping rule
        const rule = mappingRules.find(r => event.type.match(r.evidenceType.replace("*", ".*")));
        if (!rule)
            continue;
