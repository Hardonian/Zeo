...
export function inferPosterior(worldSpec, observations, seed, sourceWeights) {
    ...
    for (const observation of canonicalObs) {
        ...
        const model = worldSpec.observationModels.find(m => observation.signalId.match(m.provenancePattern.replace(/\*/g, ".*")));
        if (!model)
            continue;
        ...
        for (const varId of model.targetVariableIds) {
            ...
            if (!varState)
                continue;
            const newBand = applyObservationToBand(varState.posteriorBand, observation, model, quality);
