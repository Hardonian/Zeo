...
export class CalibrationEngine {
  ...
  computeCalibrationBuckets(
    ...
    for (let start = 0; start < 1; start += bucketSize) {
      ...
      const bucketForecasts = forecasts.filter(
  ...
  generateReport(): CalibrationReport {
    ...
    // By claim type
    const byClaimType: Record<string, ScoreResult> = {};
    const types = ["fact", "belief", "assumption"] as const;
   ...
   */
  private computeReliability(forecasts: ForecastRecord[]): number {
   ...
   */
  private computeResolution(forecasts: ForecastRecord[]): number {
    if (forecasts.length === 0) return 0;
   ...
   */
  private computeUncertainty(forecasts: ForecastRecord[]): number {
    if (forecasts.length === 0) return 0;
   ...
   */
  private computeTrends(): Array<{
    period: string;
    brierScore: number;
    calibrationError: number;
  }> {
    // Group by month
    const byMonth = new Map<string, ForecastRecord[]>();
    ...
    const trends: Array<{ period: string; brierScore: number; calibrationError: number }> = [];

    for (const [month, forecasts] of byMonth) {
      const buckets = this.computeCalibrationBuckets(forecasts);
      const avgError =
        buckets.reduce((sum, b) => sum + b.calibrationError, 0) /
        (buckets.length || 1);
  ...
  private generateRecommendations(
    ...
    if (overconfident) {
      recommendations.push(
        "Overconfidence detected: High confidence forecasts occur less often than predicted. Consider widening uncertainty bands."
      );
    }
    ...
    if (underconfident) {
      recommendations.push(
        "Underconfidence detected: Events occur more often than predicted. Consider narrowing uncertainty when evidence is strong."
      );
    }
    ...
    if (overall.sampleSize < 30) {
      recommendations.push(
        `Limited sample size (${overall.sampleSize} forecasts). Calibration estimates have high variance.`
      );
    }
    ...
    if (overall.brierScore > 0.25) {
      recommendations.push(
        `Brier score ${overall.brierScore.toFixed(3)} suggests significant room for improvement in probability assessment.`
      );
    } else if (overall.brierScore < 0.1) {
      recommendations.push(
        `Brier score ${overall.brierScore.toFixed(3)} indicates well-calibrated forecasts. Maintain current practices.`
      );
    }
