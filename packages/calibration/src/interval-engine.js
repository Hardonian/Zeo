...
export class IntervalCalibrationEngine {
     ...
     */
    addPointForecast(record) {
        this.pointForecasts.push(record);
    }
    /**
     * Add an interval forecast for coverage testing.
     */
    addIntervalForecast(record, predictedInterval, actualOutcome, lens, assumptionTypes = []) {
     ...
     */
    computeBrierScore(forecasts = this.pointForecasts) {
     ...
     */
    computeLogScore(forecasts = this.pointForecasts) {
        if (forecasts.length === 0)
            return 0;
    ...
    testIntervalCoverage(options = {}) {
        ...
        for (let i = 0; i < opts.bucketCount; i++) {
            ...
            if (bucketForecasts.length < opts.minBucketSize)
                continue;
            // Test: what % of these intervals actually contained the outcome?
            const covered = bucketForecasts.filter(f => f.actualOutcome >= f.predictedInterval.low &&
                f.actualOutcome <= f.predictedInterval.high).length;
            const coverageRate = covered / bucketForecasts.length;
            const coverageError = coverageRate - expectedCoverage;
            const avgIntervalWidth = bucketForecasts.reduce((sum, f) => sum + (f.predictedInterval.high - f.predictedInterval.low), 0) / bucketForecasts.length;
            buckets.push({
    ...
    computeCalibrationAdjustment() {
        ...
        const weightedCoverageError = buckets.reduce((sum, b) => sum + b.coverageError * b.count, 0) / totalForecasts;
     ...
     */
    generateExtendedReport(options = {}) {
        const opts = { ...defaultOptions, ...options };
        // Point calibration
     ...
     */
    applyCalibrationAdjustment(interval) {
        const adjustment = this.computeCalibrationAdjustment();
    ...
    computeReliability() {
        ...
        if (this.pointForecasts.length === 0)
            return 0;
    ...
    computeCalibrationBuckets() {
        ...
        for (let start = 0; start < 1; start += bucketSize) {
            ...
            const bucketForecasts = this.pointForecasts.filter(f => f.probability >= start && f.probability < end);
    ...
    computeScoreResultFromIntervals(forecasts) {
...
//# sourceMappingURL=interval-engine.js.map