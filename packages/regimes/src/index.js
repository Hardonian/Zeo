...
function computeRollingStats(points, windowSize) {
    const result = [];
...
function computeCusum(points, targetMean, targetStd) {
    const cusum = [];
    let positiveSum = 0;
    let negativeSum = 0;
...
function detectCusumChangePoints(points, config) {
    if (points.length < config.minWindowSize)
        return [];
...
function detectVolatilityBreaks(points, config) {
    if (points.length < config.minWindowSize * 2)
        return [];
    const recentValues = points.slice(-config.maxWindowSize).map(p => p.v);
...
function ksStatistic(sample1, sample2) {
    ...
    while (i < sorted1.length && j < sorted2.length) {
        ...
        const diff = Math.abs((i + 1) / sorted1.length - (j + 1) / sorted2.length);
...
function detectDistributionShifts(points, config) {
    if (points.length < config.minWindowSize * 2)
        return [];
    const shifts = [];
...
function detectCadenceShifts(eventTimes, config) {
    if (eventTimes.length < config.minWindowSize)
        return [];
...
function computeConfidence(sampleSize, signalStrength, config) {
...
function computeSeverity(changeMagnitude, volatilityRatio) {
...
function safeGetTimestamp(points, index, offset) {
    const idx = Math.max(0, index + offset);
    return points[idx]?.t ?? points[points.length - 1]?.t ?? new Date().toISOString();
}
export function detectRegimes(domain, numericSeries, eventTimes, signalIds = [], config) {
...
export function createRegimeState(domain, label, parameters) {
    return {
        domain,
        currentLabel: label,
...
export function createRegimeEvent(domain, kind, window, signalIds, severity, confidence, notes) {
    return {
        id: generateId(),
...
export function estimateTransitionMatrix(history) {
    ...
    const matrix = stateLabels.map((fromState, i) => {
...
function predictNextRegime(currentLabel, transitionMatrix) {
...
export function computeVolatilityTrend(numericSeries, shortWindow = 10, longWindow = 30) {
    if (numericSeries.length < longWindow)
        return 0;
    const shortStats = computeRollingStats(numericSeries.slice(-shortWindow - 1, -1), shortWindow);
    const longStats = computeRollingStats(numericSeries.slice(-longWindow - 1, -1), longWindow);
...
export function computeMeanTrend(numericSeries, shortWindow = 10, longWindow = 30) {
    if (numericSeries.length < longWindow)
        return 0;
    const shortMean = computeMean(numericSeries.slice(-shortWindow).map(p => p.v));
    const longMean = computeMean(numericSeries.slice(-longWindow).map(p => p.v));
    return (shortMean - longMean) / (Math.abs(longMean) || 1);
}
export function detectEarlyWarnings(numericSeries, config) {
    const fullConfig = { ...DEFAULT_CONFIG, ...config };
    const warnings = [];
...
export function predictRegime(domain, numericSeries, history, timeHorizonHours = 24, config) {
    ...
    const warningSeverity = earlyWarnings.reduce((max, w) => {
    ...
    const confidenceBand = computeConfidence(numericSeries.length, Math.min(1, transitionProbability), { ...DEFAULT_CONFIG, ...config });
    return {
        predictedRegime,
...
export function computeRegimeStability(states) {
    if (states.length < 2) {
        return { score: 1.0, label: "stable" };
    }
    const transitions = states.slice(1).filter((s, i) => s.currentLabel !== states[i].currentLabel).length;
