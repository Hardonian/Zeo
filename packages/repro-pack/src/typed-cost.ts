...
export function validateCost(cost: TypedCost): string[] {
    const errors: string[] = [];
...
export function validateDuration(duration: TypedDuration): string[] {
    const errors: string[] = [];
...
export function validateRisk(risk: TypedRisk): string[] {
    const errors: string[] = [];
 ...
 */
export function durationToMinutes(d: TypedDuration): number {
 ...
 */
export function parseCost(raw: string): TypedCost {
 ...
 */
export function parseDuration(raw: string): TypedDuration {
    const match = raw.trim().match(/^([\d.]+)\s+(.+)$/);
    if (!match) {
        throw new Error(`Cannot parse duration string: "${raw}"`);
    }
    const unit = match[2]!.toLowerCase().replace(/s$/, "") as TypedDuration["unit"];
    const validUnits = ["second", "minute", "hour", "day", "week", "month"];
    const mapped = validUnits.includes(unit) ? (unit + "s") as TypedDuration["unit"] : unit;
...
let _planEventId = 0;
function nextPlanEventId(): string {
...
export function checkPlanFeasibility(
    planId: string,
    planCost: TypedCost,
    planDuration: TypedDuration,
    planRisk: TypedRisk,
    budget: BudgetConstraints,
): { result: PlanResult; events: RunEvent[] } {
    const events: RunEvent[] = [];
    const reasons: string[] = [];
    const violated: Array<{ constraint: string; required: number; available: number; unit: string }> = [];
    const relaxation: Array<{ constraint: string; relaxTo: number; unit: string }> = [];
    ...
    if (budget.maxDuration) {
        ...
        if (planMinutes > budgetMinutes) {
            reasons.push(
                `Duration ${planDuration.amount} ${planDuration.unit} exceeds budget ${budget.maxDuration.amount} ${budget.maxDuration.unit}`,
            );
            violated.push({
                constraint: "duration",
