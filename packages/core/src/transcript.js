...
export function normalizeTranscriptForReplay(transcript) {
    return {
...
function buildDecisionBoundaries(evaluations) {
...
function normalizeConfidence(result) {
    ...
    if (intervals.length === 0)
        return { lower: "0.0000", upper: "1.0000", method: "range_aggregation" };
...
function normalizeDecisionResultForHash(result) {
    return {
...
function buildTranscript(input, result) {
...
export function finalizeDecisionTranscript(transcript) {
...
export function verifyDecisionTranscript(transcript) {
    const { transcript_hash, transcript_id, ...unsigned } = transcript;
    const recomputed = computeTranscriptHash(unsigned);
    const reasons = [];
    if (transcript_hash !== recomputed)
        reasons.push("transcript_hash_mismatch");
    if (transcript_id !== `tr_${transcript_hash.slice(0, 16)}`)
        reasons.push("transcript_id_mismatch");
    const replayNormalized = normalizeTranscriptForReplay(unsigned);
    if (replayNormalized.flip_distances.length === 0)
        reasons.push("missing_flip_distances");
    if (replayNormalized.decision_boundaries.length === 0)
        reasons.push("missing_decision_boundaries");
