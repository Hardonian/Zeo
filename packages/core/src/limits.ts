 ...
 */
export function validateSize(obj: unknown, limitBytes: number, label: string): void {
 ...
 */
export function checkDepth(obj: unknown, depth = 0, max = ZEO_LIMITS.MAX_JSON_DEPTH): void {
    if (depth > max) {
        throw new Error(`Limit Exceeded: JSON structure too deep (max ${max})`);
    }
    if (obj && typeof obj === 'object') {
        for (const key in obj) {
            checkDepth((obj as any)[key], depth + 1, max);
        }
    }
}
