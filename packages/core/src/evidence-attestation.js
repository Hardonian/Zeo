 ...
 */
export function signManifest(manifestHash, mode, secretOrPrivateKey) {
    if (mode === "none")
        return undefined;
    if (!secretOrPrivateKey)
        throw new Error("Missing secret for signing");
    if (mode === "hmac") {
 ...
 */
export function verifyManifestSignature(manifestHash, signature, mode, secretOrPublicKey) {
    if (mode === "hmac") {
