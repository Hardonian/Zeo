export function generateDecisionReport(result, runData) {
    const sections = [];
    // 1. Executive Summary
    ...
    if (evaluationsSection)
        sections.push(evaluationsSection);
...
function buildRationaleSection(result, runData) {
    const citations = [];
    let content = "";
    if (result.explanation && result.explanation.why.length > 0) {
...
function buildAlternativesSection(result, runData) {
    if (!result.explanation?.whatWouldChange?.length)
        return null;
    let content = "";
    const citations = [];
...
function buildAssumptionsSection(result, runData) {
    ...
    if (!assumptions?.length && !uncertaintyMap)
        return null;
    let content = "";
...
function buildBudgetSection(result) {
    if (!result.usage && !result.budget)
        return null;
    let content = "";
    if (result.usage) {
...
function buildEvaluationsSection(result) {
...
export function renderDecisionSummary(result) {
