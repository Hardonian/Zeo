    ...
    validate(context) {
        ...
        if (context.assumptions) {
            context.assumptions.forEach((assumption) => {
                ...
                const val = typeof ass.value === "number"
                    ? ass.value
                    : undefined;
...
class UncertaintyHonestyPolicy {
    ...
    validate(context) {
        ...
        if (context.runData?.uncertaintyMap) {
            Object.entries(context.runData.uncertaintyMap).forEach(([key, uncertainty]) => {
                const uncertaintyValue = uncertainty;
...
class ConstraintFeasibilityPolicy {
    ...
    validate(context) {
        ...
        const isInfeasible = context.runData?.events?.some((event) => event.type === "PLAN_INFEASIBLE");
        if (isInfeasible) {
            violations.push({
                code: "POLICY_VIOLATION",
                policyId: this.id,
