...
export function computeDeterministicSeed(decisionHash, observationHash, depth) {
    const combined = `${decisionHash}:${observationHash || "no-observations"}:${depth}`;
    return createHash("sha256").update(combined).digest("hex");
}
export function computeRunSeed(decisionSpecHash, observationBatchHash, depth, maxBranches) {
    const components = [
        decisionSpecHash,
