...
export type DecisionTranscript = {
    ...
    analysis: {
        flip_distances: Array<{
            assumption_id: UUID;
            distance: string;
            boundary: string;
        }>;
        decision_boundaries: Array<{
            lens: LensId;
            robust_actions: UUID[];
            fragile_assumptions: UUID[];
        }>;
        voi_rankings: Array<{
            prompt: string;
            rank: number;
    ...
    plan: {
        regret_bounded_evidence_plan: Array<{
            prompt: string;
            rationale: string;
        }>;
...
export type RegimeKind = "change_point" | "volatility_break" | "mean_shift" | "distribution_shift" | "cadence_shift";
