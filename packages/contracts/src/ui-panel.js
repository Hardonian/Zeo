 ...
 */
export async function verifyManifestSignature(manifest, secretKey) {
    try {
 ...
 */
export function verifyManifestSignatureSync(_manifest, _secretKey) {
    try {
 ...
 */
export function isSecurityCompliant(manifest) {
 ...
 */
export function getSecurityDenialReason(manifest) {
    if (manifest.kind === "react") {
        return null;
    }
    const signed = manifest;
 ...
 */
export function isValidPermissionRequest(manifest, capability) {
 ...
 */
export function createPermissionResponse(capability, granted, grantId) {
    const payload = {
        capability,
        granted,
