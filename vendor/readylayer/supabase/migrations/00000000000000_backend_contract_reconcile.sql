-- ============================================
-- ReadyLayer Backend Contract Reconciliation Migration
-- ============================================
-- 
-- This is the canonical, idempotent migration bundle that ensures
-- the live Supabase database matches the expected backend contract.
-- 
-- Safe to run multiple times - uses IF NOT EXISTS patterns and
-- guards to prevent errors on re-execution.
-- 
-- Generated: 2024-12-30
-- Purpose: Reconcile live database state with expected contract
-- 
-- ============================================
...
CREATE OR REPLACE FUNCTION public.has_org_role(org_id TEXT, required_role TEXT) 
RETURNS BOOLEAN 
LANGUAGE SQL 
STABLE 
SECURITY DEFINER
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_schema = 'public' AND table_name = 'User' AND column_name = 'emailVerified') THEN
...
BEGIN
    FOR table_record IN 
        ...
        AND tablename IN (
            ...
            'Installation', 'RepositoryConfig', 'OrganizationConfig',
