...
export default async function RunsPage({
  ...
  return (
    <div className="space-y-4">
      ...
      <form action={createRun as unknown as string} className="rounded border bg-white p-4"><input className="rounded border px-3 py-2" name="projectId" placeholder="Project ID (optional)" /><button className="ml-2 rounded bg-blue-600 px-3 py-2 text-white">Create run</button></form>
      <form action={appendEvent as unknown as string} className="rounded border bg-white p-4 space-x-2"><input className="rounded border px-3 py-2" name="runId" placeholder="Run ID" required aria-label="Run ID" /><input className="rounded border px-3 py-2" name="eventType" defaultValue="log" aria-label="Event Type" /><input className="rounded border px-3 py-2" name="payload" defaultValue='{"message":"ok"}' aria-label="Payload" /><button className="rounded bg-blue-600 px-3 py-2 text-white">Append event</button></form>
      <form action={addArtifact as unknown as string} className="rounded border bg-white p-4 space-x-2"><input className="rounded border px-3 py-2" name="runId" placeholder="Run ID" required aria-label="Run ID for artifact" /><input className="rounded border px-3 py-2" name="filename" placeholder="artifact.json" aria-label="Filename" /><button className="rounded bg-blue-600 px-3 py-2 text-white">Create artifact record</button></form>
